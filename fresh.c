...
uint32_t decode_utf8_fast(const unsigned char *s, int *len) {       // Декодер
    unsigned char c = s[0];
...
int get_vis_width(uint32_t cp) {
    ...
    return 1; }
void smart_append(int row, const char *key) {
    int len; uint32_t cp = decode_utf8_fast((const unsigned char*)key, &len);
    if ((unsigned char)key[0] < 32 && key[1] == 0) return;            // Игнорируем функциональные клавиши из GetKey
...
void CRD(int row) {
    Cell *attr_ptr = (Cell*)GET_ATTR(row, 0);
    for (size_t i = 0; i < (GLOBAL_SIZE_STR / sizeof(Cell)); i++) attr_ptr[i] &= CLEAN_MASK; }
// Глобальные координаты Лупы (Viewport) — наш "фонарик"
int view_x = 0, view_y = 0;
void render_line_auto(int world_row, int screen_row) {
    if (world_row < 0 || world_row >= GLOBAL_STRING) { printf("\033[%d;1H%s%s", screen_row + 1, LWOff, ELin); return; }
    int w, h; w = GetWH(&h); (void)w;
    LineData *ld = GET_DLINE(world_row);
    Token *t_ptr = GET_TOKEN(world_row);
    char *data_ptr = GET_BLINE(world_row);
    int cur_v_x = 0, byte_off = 0, i = 0;
    while (i < ld->count && cur_v_x < view_x) { byte_off += t_ptr[i].len; cur_v_x += t_ptr[i].visual; i++; }
    printf("\033[%d;1H" LWOff, screen_row + 1); if (cur_v_x > view_x) printf(" ");
    if (ld->len > byte_off) fwrite(data_ptr + byte_off, 1, ld->len - byte_off, stdout);
    printf(ELin); CRD(world_row); }

void refresh_world(int cur_x, int cur_y) {
    int w, h; w = GetWH(&h);
    if (cur_x < view_x) view_x = (cur_x < 0) ? 0 : cur_x;
    if (cur_x >= view_x + w) view_x = cur_x - w + 1;
    if (cur_y < view_y) view_y = (cur_y < 0) ? 0 : cur_y;
    if (cur_y >= view_y + h) view_y = cur_y - h + 1;
    printf(HCur); for (int i = 0; i < h; i++)  render_line_auto(view_y + i, i);
    if (cur_x >= 0 && cur_y >= 0) printf("\033[%d;%dH" ShCur, (cur_y - view_y) + 1, (cur_x - view_x) + 1);
    fflush(stdout); }
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    if (k[0] == 27) {
        ...
        if (k[1] == K_RIG) cur_x++;
        if (k[1] == K_LEF) cur_x--; }
    else { smart_append(cur_y, k); cur_x += get_vis_width(decode_utf8_fast((const unsigned char*)k, &len)); }
